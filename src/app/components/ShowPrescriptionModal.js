...
const getFlatList = (prescriptions, handleClose) => {
  return (
    <FlatList
      ListHeaderComponent={
        <Pressable className="items-end" onPress={() => handleClose()}>
          <View className="border-b-2 border-gray-300">
            <Text className="border-2 p-1 mb-2 border-gray-300 rounded-xl text-lg text-center">
              X
            </Text>
          </View>
        </Pressable>
      ...
      renderItem={({ item }) => (
        <View className="flex-row flex-3 w-full justify-between border-b-2 border-gray-300 gap-x-20 android:gap-x-2 ios:gap-x-2">
          <View className="flex-col flex-2 android:flex-1 ios:flex-1">
            <Text className="flex-1 text-lg text-start">
              Prescription Date:{" "}
              {new Date(item.prescriptionDate).toLocaleDateString("tr-TR")}
            </Text>
            <Text className="flex-1 text-lg text-start">
              Expiry Date:{" "}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View className="flex-row flex-3 justify-between border-b-2 border-gray-300 p-2 w-full">
                  ...
                  <Text className="flex-1 text-lg text-end">
                    {item.frequency}
                  </Text>
                </View>
              )}
